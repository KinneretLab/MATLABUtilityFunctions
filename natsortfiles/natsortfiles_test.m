...
%
% See also NATSORTFILES TESTFUN_NSX ARBSORT_TEST NATSORT_TEST NATSORTROWS_TEST
fnh = @natsortfiles;
chk = testfun_nsx(fnh);
%
...
chk(Aa, fnh, {'a1.txt','a2.txt','a10.txt'})
chk(Aa, fnh, {'a1.txt','a2.txt','a10.txt'}, [3,1,2]) % not in Mfile
%
Ab =         {'test2.m';'test10-old.m';'test.m';'test10.m';'test1.m'};
chk(Ab, fnh, {'test.m';'test1.m';'test2.m';'test10.m';'test10-old.m'})
chk(Ab, fnh, {'test.m';'test1.m';'test2.m';'test10.m';'test10-old.m'}, [3;5;1;4;2]) % not in Mfile
%
Ac =         {'A2-old\test.m';'A10\test.m';'A2\test.m';'A1\test.m';'A1-archive.zip'};
chk(Ac, fnh, {'A1\test.m';'A1-archive.zip';'A2\test.m';'A2-old\test.m';'A10\test.m'})
chk(Ac, fnh, {'A1\test.m';'A1-archive.zip';'A2\test.m';'A2-old\test.m';'A10\test.m'}, [4;5;3;1;2]) % not in Mfile
%
Ad =         {'A1\B','A+/B','A/B1','A=/B','A\B0'};
chk(Ad, fnh, {'A\B0','A/B1','A1\B','A+/B','A=/B'})
chk(Ad, fnh, {'A\B0','A/B1','A1\B','A+/B','A=/B'}, [5,3,1,2,4]) % not in Mfile
%
Af =         {'test_new.m';'test-old.m';'test.m'};
chk(Af, fnh, {'test.m';'test-old.m';'test_new.m'})
chk(Af, fnh, {'test.m';'test-old.m';'test_new.m'}, [3;2;1]) % not in Mfile
%
%% HTML Examples %%
%
Aa =         txf({'a2.txt','a10.txt','a1.txt'});
chk(Aa, fnh, txf({'a1.txt','a2.txt','a10.txt'}))
chk(Aa, fnh, @i, [3,1,2]) % Not in HTML
chk(Aa, fnh, @i, [3,1,2], {{'a',2;'a',10;'a',1},{'.txt';'.txt';'.txt'}}) % not in HTML
chk(Aa, fnh, @i,      @i, {{'a',2;'a',10;'a',1},{'.txt';'.txt';'.txt'}}) % not in HTML
%
P = 'natsortfiles_test';
unzip(sprintf('%s.zip',P))
...
chk(Ab,              fnh, txf({'1.2.txt','1.3.txt','1.10.txt'}), [3,1,2]) % index not in HTML
chk(Ab, '\d+\.?\d*', fnh, txf({'1.10.txt','1.2.txt','1.3.txt'}), [2,3,1]) % index not in HTML
%
chk({'natsort_doc.html','natsortrows_doc.html','..','.'}, [], 'rmdot', fnh, ...
	{'natsort_doc.html','natsortrows_doc.html'})
%
Ac =                               txf({'1.9','1.10','1.2'});
chk(Ac, '\d+\.?\d*',          fnh, txf({'1.2','1.9','1.10'}), [3,1,2]) % index not in HTML
chk(Ac, '\d+\.?\d*', 'noext', fnh, txf({'1.10','1.2','1.9'}), [2,3,1]) % index not in HTML
%
Ad =                      txf({'B/3.txt','A/1.txt','B/100.txt','A/20.txt'});
chk(Ad,              fnh, txf({'A/1.txt','A/20.txt','B/3.txt','B/100.txt'}), [2,4,1,3]) % index not in HTML
chk(Ad, [], 'xpath', fnh, txf({'A/1.txt','B/3.txt','A/20.txt','B/100.txt'}), [2,1,4,3]) % index not in HTML
%
Ae =                         txf({'B.txt','10.txt','1.txt','A.txt','2.txt'});
chk(Ae, [],  'descend', fnh, txf({'B.txt','A.txt','10.txt','2.txt','1.txt'}))
chk(Ae, [], 'char<num', fnh, txf({'A.txt','B.txt','1.txt','2.txt','10.txt'}))
%
Af =         txf({'abc2xyz.txt','abc10xyz.txt','abc2xyz.txt','abc1xyz.txt'});
chk(Af, fnh, txf({'abc1xyz.txt','abc2xyz.txt','abc2xyz.txt','abc10xyz.txt'}), [4,1,3,2])  % index not in HTML
chk(Af, fnh, txf({'abc1xyz.txt','abc2xyz.txt','abc2xyz.txt','abc10xyz.txt'}), [4,1,3,2], ...index not in HTML
	{{'abc',2,'xyz';'abc',10,'xyz';'abc',2,'xyz';'abc',1,'xyz'},...
	{        '.txt';        '.txt';       '.txt';       '.txt'}})
%
chk({'test_ccc.m';'test-aaa.m';'test.m';'test.bbb.m'}, fnh,... Ag
	{'test.m';'test-aaa.m';'test.bbb.m';'test_ccc.m'}, [3;2;4;1]) % index not in HTML
chk({'test2.m';'test10-old.m';'test.m';'test10.m';'test1.m'}, fnh,... Ah
	{'test.m';'test1.m';'test2.m';'test10.m';'test10-old.m'}, [3;5;1;4;2]) % index not in HTML
chk({'A2-old\test.m';'A10\test.m';'A2\test.m';'AXarchive.zip';'A1\test.m'}, fnh,... Ai
...
chk(Aj, fnh,...
	txf({'1.23V.csv','1.200V.csv','010V.csv','+1.csv','-1V.csv'}), [1,5,4,3,2]) % index not in HTML
chk(Aj, '[-+]?\d+\.?\d*', fnh,...
	txf({'-1V.csv','+1.csv','1.200V.csv','1.23V.csv','010V.csv'}), [2,3,5,1,4]) % index not in HTML
%
Ak = txf({'1,3.txt', '1,10.txt', '1,2.txt'});
chk(Ak, '\d+,?\d*', fnh,...
	txf({'1,10.txt', '1,2.txt', '1,3.txt'}))
chk(Ak, '\d+,?\d*', fnh, @i, [2,3,1], {{1.3;1.1;1.2},{'.txt';'.txt';'.txt'}}) % not in HTML
%
